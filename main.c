...
int main(int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (strspn(argv[i], "0123456789") == len) {
      char *end = NULL;
      uint32_t asn = htonl(strtol(argv[i], &end, 10));
      if (asn > 0 && end == (argv[i] + len))
        lr = mmdb_lookup4(asn_db, (uint8_t *)&asn);
    } else if (strspn(argv[i], ".0123456789") == len &&
               inet_pton(AF_INET, argv[i], &bip4)) {
      lr = mmdb_lookup4(pfx_db, (uint8_t *)&bip4);
    } else if (strspn(argv[i], ":0123456789abcdefABCDEF") == len &&
               inet_pton(AF_INET6, argv[i], &bip6)) {
